...
PRIME_PERFORMANCE_DECLARATIONS(dc_miller);

GMP_TEST_BEGIN(dc_miller,	u,x,x, x, b,x,x, 1)
expected = mpz_probab_prime_p(gmp_a, 25);
if (expected == 2) expected = 1;
r = dc_miller(a);
GMP_TEST_END(dc_miller,		u,x,x, x, b,x,x, 1)
...
int main(int argc, char **argv)
{
	...
	quick_tests {
		for (i = 20; i <= 64; i += 8)
			dc_miller_gmp_test(100000, i, 0);
		dc_miller_gmp_test(100000, 64, 0);
	}
	...
	perf_tests {
		if (perf_test_p) {
			for (i = 2; i <= 64; i++)
				dc_miller_p_performance(100000, i);
		} else {
			for (i = 1; i <= 64; i++)
				dc_miller_performance(100000, i);
		}
	}
